...
class UsersController extends ActiveController
{
    ...
    public function actionCreate()
    {
        ...
        if (empty($user_pass) || empty($user_email) || empty($user_name) ) {
            Yii::$app->response->statusCode=400;
                echo json_encode(array('status'=>400,'error'=>array('message'=>"Some fields are missing.")));
        } else {

            $rows_email = $user->getUserByEmail($user_email);
            $rows_username = $user->getUserByUsername($user_name);
    ...
    public function actionRequestRegistrationToken()
    {
        ...
        $user_email = $request->post('user_email');
        $status = 'Unverified';
        $model = $user->getUserByStatus($user_email, $status);
    ...
    public function actionVerifyRegistration()
    {
        ...
        $user = new users();
        $model = $user->getUserByStatus($user_email, $current_status);
        ...
        if ($model) {
            if ($model['reset_token']!==null && $model['reset_token'] == $code) {
                 if ($model['reset_expiry']>=$current) {
                    ...
                    if($token_update) {
                        header("Location: http://clients2.5stardesigners.net/RD/onboarding/test.php");
                        exit();
                    } else {
                        Yii::$app->response->statusCode=400;
                        echo json_encode(array(
    ...
    public function actionLogin()
    {
        ...
        $status = 'Unverified';
        $user = new users();
        $model=$user->getUserByEmail($user_email);
        ...
        if ($model) {
            ...
            if ($is_password_correct) {
                if ($model['status']== 'Unverified') {
                    ...
                    if ($model_token) {
                        $userdata = $user->getUserByEmail($user_email);
